...
svc=SVC(C=10, kernel="rbf", gamma=0.05, class_weight="balanced", random_state=50)
svc.fit(X_train, y_train)
score=svc.score(X_test, y_test)
print("Score {0:.4f}".format(score))
  
"""labels= svc.predict(test)
plt.scatter(X_train[:2000,0], X_train[:2000,1] , edgecolor='none',  alpha=0.5, cmap=plt.get_cmap('jet', 10), s=5)
plt.colorbar()"""
